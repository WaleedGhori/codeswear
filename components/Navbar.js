...
const Navbar = ({ logout, user, cart, subtotal, addToCart, removeToCart, clearCart }) => {
  ...
  return (
    <>
      {!sidebar && <span className="right-14 top-5 z-30 fixed" >
        {dropDown &&
          <div onMouseOver={() => setDropDown(true)} onMouseLeave={() => setDropDown(false)} className="absolute right-6 py-4 bg-pink-50 shadow-lg top-5 rounded-md px-5 w-32 z-30">
            <ul>
